...
import { useEthersProvider } from "~~/hooks/scaffold-eth/Adapters";
import getGasFee from "~~/utils/account-abstraction/getGasFee";
import { getTargetNetwork } from "~~/utils/scaffold-eth";

const Home: NextPage = () => {
  const { scwAddress, simpleAccountAPI, sendSimpleAccountUserOp } = useSimpleAccount();
    ...
    try {
      ...
      if (simpleAccountAPI && YourContract && targetNetwork.rpcUrls.alchemy?.http[0]) {
        ...
        const op = await simpleAccountAPI.createSignedUserOp({
          ...
          data: callData,
          ...getGasFee(ethersProvider),
        });
        const receipt = await sendSimpleAccountUserOp({ userOp: op });
        console.log("Ther transaction receipt is: ", receipt);
      }
    } catch (error) {
